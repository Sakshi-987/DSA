...
public class modifiedBFS {
    ...
    // this modified bfs approach is valid to all types of undirected
    // unweighted/weihted graphs
    public static ArrayList<Integer> bfs(ArrayList<Edge>[] graph) {
        boolean[] vis = new boolean[graph.length];
        ArrayList<Integer> output = new ArrayList<>();
        for (int i = 0; i < graph.length; i++) {
            if (!vis[i])
                bfsUtil(graph, vis, output,i);
        }
        return output;
    }

    public static void bfsUtil(ArrayList<Edge>[] graph, boolean[] vis, ArrayList<Integer> output, int index) {
        ...
        while (!q.isEmpty()) {
            ...
            if (!vis[curr]) {
                ...
                // }
                // }
                // Edge isliye because arraylist is of edge type
